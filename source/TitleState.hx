...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var bg:FlxSprite = new BGSprite('titleBG');
		add(bg);
		...
		smoke = new FlxSprite(750, -25);
		smoke.frames = Paths.getSparrowAtlas('smoke');
		smoke.animation.addByPrefix('smoke', 'smoke', 8);
		smoke.animation.play('smoke');
		smoke.setGraphicSize(Std.int(smoke.width * 2));
		smoke.alpha = 0.2;
		smoke.updateHitbox();
		...
		if(!closedState) {
			switch (curBeat)
			{
				...
				case 4:
					deleteCoolText();
				case 5:
					createCoolText(['Made by', 'Vullen']);
				case 7:
					addMoreText('using Psych Engine');
				case 8:
					deleteCoolText();
					logoSpr.visible = false;
				case 9:
