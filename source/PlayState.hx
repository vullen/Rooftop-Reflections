...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'rooftop': //Week D - Drizzle, Pieces
				var bg:BGSprite = new BGSprite('rooftop/bg', -670, -450, 0.2, 0.2);
				bg.setGraphicSize(Std.int(bg.width * 0.6));
				add(bg);

				var roof:BGSprite = new BGSprite('rooftop/roof', -500, -20);
				roof.setGraphicSize(Std.int(roof.width * 0.5));
				roof.updateHitbox();
				add(roof);

				rain = new FlxSprite(600, 100);
				rain.frames = Paths.getSparrowAtlas('rain');
				rain.animation.addByPrefix('rain', 'rain', 40, false);
				rain.setGraphicSize(Std.int(rain.width * 0.1));
				rain.alpha = 0.4;
				rain.scrollFactor.set(0.9, 0.9);
				rain.updateHitbox();
				add(rain);

				overlay = new FlxSprite(-700, -100).makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), FlxColor.MAGENTA);
				overlay.alpha = 0.4;
				overlay.blend = 'multiply';

			case 'storm': //Week D - bang
				stormBG = new BGSprite('storm/bg', -670, -450, 0.2, 0.2, ['bg']);
				stormBG.animation.addByPrefix('end', 'end', 18, false);
				stormBG.animation.play('bg');
				stormBG.setGraphicSize(Std.int(stormBG.width * 0.6));
				add(stormBG);

				var roof:BGSprite = new BGSprite('storm/roof', -500, -20, 0.95, 0.95);
				roof.setGraphicSize(Std.int(roof.width * 0.5));
				roof.updateHitbox();
				add(roof);

				smoke = new FlxSprite(314, 10);
				smoke.frames = Paths.getSparrowAtlas('storm/smoke');
				smoke.animation.addByPrefix('idle', 'idle', false);
				smoke.animation.addByPrefix('smoke', 'smoke', 8, true);
				smoke.animation.play('idle');
				smoke.setGraphicSize(Std.int(smoke.width * 0.5));
				smoke.alpha = 0.2;
				smoke.scrollFactor.set(0.2, 0.2);
				smoke.updateHitbox();
				add(smoke);
				...
				overlay = new FlxSprite(-700, -100).makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), FlxColor.PURPLE);
				overlay.alpha = 0.5;
				overlay.blend = 'multiply';

				flash = new FlxSprite(-700, -100).makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), FlxColor.WHITE);
				flash.alpha = 0.4;
				flash.blend = 'add';
		...
		if (curStage != 'rooftop' && curStage != 'storm') {
			add(gfGroup);
		}
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			{
				if (Paths.formatToSongPath(SONG.song) != 'drizzle')
				{
	...
	override function beatHit()
	{
		...
		if (curBeat > 19 || Paths.formatToSongPath(SONG.song) != 'drizzle') {
			if(curBeat % 2 == 0) {
		...
		if (curBeat == 336 && Paths.formatToSongPath(SONG.song) == 'bang' && curStage == 'storm')
			{
				FlxG.sound.play(Paths.sound('thunder_1'));
				FlxG.camera.shake(0.02, 0.3, Y);
				stormBG.animation.play('end');
			...
			if (curBeat % 2 == 0)
			{
				if ((curStage == 'rooftop' && (FlxG.random.int(0, 1) == 1)) || curStage == 'storm') 
				{
					rain.offset.set(FlxG.random.int(-350, -25) + (FlxG.random.int(0, 1) * 750), FlxG.random.int(100, 300));
			...
			if (curBeat % 2 == 1)
			{
				if ((curStage == 'rooftop' && (FlxG.random.int(0, 1) == 1)) || curStage == 'storm') 
				{
					rain.offset.set(FlxG.random.int(-725, -400) + (FlxG.random.int(0, 1) * 750), FlxG.random.int(100, 300));
					rain.animation.play('rain');
				}
			}
